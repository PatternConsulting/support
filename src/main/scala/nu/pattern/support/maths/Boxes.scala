...
//
//    val intersections = mutable.LinkedList.empty[Box]
//
//    entries.foreach { entry =>
//      val box = entry.geom.toBox
//
//    }
//
//    intersections.toSet.map(ev.to)
//  }

//  private def toPoints(geom: Geom): Seq[Point] =
//    Point(geom.x, geom.y) ::
//      Point(geom.x2, geom.y) ::
//      Point(geom.x, geom.y2) ::
//      Point(geom.x2, geom.y2) ::
//      Nil

   def intersection[T](b0: Box, b1: Box) = {
  ...
  private def union(b0: Box, b1: Box) = {
    val x0 = Math.min(b0.x, b1.x)
    val y0 = Math.min(b0.y, b1.y)

    val x1 = Math.max(b0.x2, b1.x2)
    val y1 = Math.max(b0.y2, b1.y2)
